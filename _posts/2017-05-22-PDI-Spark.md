...
| PDI | **AEL Spark executor** | `HDFS_SPARK_EXECUTOR_LOCATION` | HDFS | includes only the required libraries needed by the Spark nodes themselves to execute a transformation
...
$ ./pdi-daemon config --daemon sparkApp $PDI_SPARK_APP_HOME/data-integration
...
$ ./pdi-daemon config --daemon --listsparkHome=/home/dsteiner/apps/spark-2.1.0-bin-hadoop2.7
...
[OPEN] Does `setenv` overwrite `etc/org.pentaho.pdi.engine.daemon.cfg`?
...
Is if YARN is the only supported resource negotiator? This seems to be the case for now.